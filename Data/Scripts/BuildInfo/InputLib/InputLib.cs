...
namespace Digi.Input
{
    ...
    public class InputLib : IDisposable
    {
        ...
        private static void AppendInputTypes(StringBuilder str, List<InputBase> list, InputTypeEnum type, string title, bool sortAlphabetically = false)
        {
            ...
            const string NEWLINE = "\r\n"; // keeping it consistent-length for removal later
            int entries = 0;

            str.Append(NEWLINE).Append("// ").Append(title).Append(": ");
            str.Append(NEWLINE).Append(LINE_START);
            ...
            if(entries == 0)
                str.Length -= (NEWLINE.Length + LINE_START.Length); // remove last line start
