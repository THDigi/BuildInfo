...
namespace Digi.BuildInfo
{
    public class Constants : ModComponent
    {
        ...
        public const bool BLOCKPICKER_IN_MP = false; // FIXME: block picker does nothing in MP, see comments in: https://support.keenswh.com/spaceengineers/general/topic/187-2-modapi-settoolbarslottoitem-causes-everyone-in-server-to-disconnect
        ...
        private void AddCharsSize(int size, string chars)
        {
        ...
        private void ParseFonts()
        {
            ...
            using(var writer = MyAPIGateway.Utilities.WriteFileInLocalStorage("FontSizes.txt", typeof(Constants)))
            {
                writer.Write(sb);
            }
        }
        ...
        {
            if(!MyAPIGateway.Utilities.FileExistsInLocalStorage(file, typeof(Constants)))
                return false;

            using(var reader = MyAPIGateway.Utilities.ReadFileInLocalStorage(file, typeof(Constants)))
            {
                string line;
        ...
        private string GetInBetween(string content, string start, string end, int startFrom = 0)
        {
            ...
            if(startIndex == -1)
                throw new Exception($"Couldn't find '{start}' after {startFrom.ToString()} in line: {content}");

            startIndex += start.Length;
            int endIndex = content.IndexOf(end, startIndex);

            if(endIndex == -1)
                throw new Exception($"Couldn't find '{end}' after {startIndex.ToString()} in line: {content}");
