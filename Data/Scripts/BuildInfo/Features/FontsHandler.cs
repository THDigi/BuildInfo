...
namespace Digi.BuildInfo.Features
{
    public class FontsHandler : ModComponent
    {
        ...
        void ComputeCharacterSizes()
        {
            ...
            AddCharsSize(32, "");
            // save some memory by just assuming any missing character is chinese
...
#if false
        public delegate Vector2 MeasureTextDelegate(StringBuilder sb, string font, float scale);
        public MeasureTextDelegate GetTextSizeInPixels;

        private BlockMonitor.CallbackDelegate LCDSpawnedCallback;

        public const string MeasureForFont = "White"; // 'fonts/white/'; for `fonts/white_shadow` use Debug

        public FontsHandler(BuildInfoMod main) : base(main)
        {
            //ParseFonts();

            LCDSpawnedCallback = LCDSpawned;

            // find any LCD block that's already in the world
        ...
        protected override void RegisterComponent()
        {
            ...
            if(GetTextSizeInPixels == null)
            {
                // no LCDs were present in world, spawn our own...
                SpawnLCDForFunction();
            }
        ...
        // HACK: need an LCD block to get the MeasureStringInPixels function because MyFontDefinition is not whitelisted.
        void LCDSpawned(IMySlimBlock slimBlock)
        {
            if(GetTextSizeInPixels != null)
                return;

            var surfaceOwner = slimBlock.FatBlock as IMyTextSurfaceProvider;
            if(surfaceOwner == null || surfaceOwner.SurfaceCount == 0)
                return;

            GetTextSizeInPixels = surfaceOwner.GetSurface(0).MeasureStringInPixels;

            Log.Info("Succesfully got MeasureStringInPixels function from existing block");
        }

        void SpawnLCDForFunction()
        {
            // DEBUG test!
            MyCubeBlockDefinition spawnDef = null;
            ...
            foreach(var def in MyDefinitionManager.Static.GetAllDefinitions())
            {
                ...
                if(blockDef.Id.TypeId == typeof(MyObjectBuilder_TextPanel))
                {
                    spawnDef = blockDef;
                    break;
                }
            ...
            if(spawnDef == null)
            {
                Log.Error("Could not find any TextPanel type block!");
                return;
            }

            new TempBlockSpawn(spawnDef, callback: BlockSpawned);
        }

        void BlockSpawned(IMySlimBlock slim)
        {
            var surfaceOwner = slim.FatBlock as IMyTextSurfaceProvider;

            if(surfaceOwner == null)
            {
                Log.Error($"{slim.BlockDefinition.Id.ToString()} is not a IMyTextSurfaceProvider");
                return;
            }

            if(surfaceOwner.SurfaceCount == 0)
            {
                Log.Error($"{slim.BlockDefinition.Id.ToString()} is not does not have any surfaces");
                return;
            }

            GetTextSizeInPixels = surfaceOwner.GetSurface(0).MeasureStringInPixels;

            Log.Info("Succesfully got MeasureStringInPixels function from spawned-in block");
        }
#endif
    }
}
