    ...
    public class Weapon : SpecializedOverlayBase
    {
        ...
        public override void Draw(ref MatrixD drawMatrix, OverlayDrawInstance drawInstance, MyCubeBlockDefinition def, IMySlimBlock block)
        {
            ...
            if(hasMuzzles)
            {
                ...
                if(ammoDef == null)
                {
                    MyAmmoMagazineDefinition mag = Main.Caches.AmmoMagDefs.GetValueOrDefault(weaponDef.AmmoMagazinesId[0]);
                    if(mag != null)
                        ammoDef = Main.Caches.AmmoDefs.GetValueOrDefault(mag.AmmoDefinitionId);
                }
