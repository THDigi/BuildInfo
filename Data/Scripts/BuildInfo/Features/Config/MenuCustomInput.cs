...
namespace Digi.BuildInfo.Features.Config
{
    ...
    public class MenuCustomInput : InputCustomBase
    {
        ...
        {
            return BuildInfoMod.Config.MenuBind.Value.IsJustPressed();
        }

        public override bool IsAssigned(ControlContext contextId = ControlContext.CHARACTER)
        {
            return BuildInfoMod.Config.MenuBind.Value.IsAssigned(contextId);
        }

        public override bool IsPressed(ControlContext contextId = ControlContext.CHARACTER)
        {
            return BuildInfoMod.Config.MenuBind.Value.IsPressed(contextId);
        }
        ...
        {
            BuildInfoMod.Config.MenuBind.Value.GetBinds(output, contextId, specialChars);
        }
    }
}
