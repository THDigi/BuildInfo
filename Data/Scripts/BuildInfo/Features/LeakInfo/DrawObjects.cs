...
namespace Digi.BuildInfo.Features.LeakInfo
{
    ...
    public class Particle
    {
        ...
        public DrawReturn Draw(IMyCubeGrid grid, List<LineI> lines, Color colorWorld, Color colorOverlay, ParticleData particleData, ref Vector3D camPos, ref Vector3D camFw, ref int delayParticles, bool spawnSync)
        {
            ...
            var pointVisible = LeakInfo.IsVisibleFast(ref camPos, ref camFw, ref posWorld);
            ...
            if(pointVisible)
            {
                var posOverlay = camPos + ((posWorld - camPos) * LeakInfo.DRAW_DEPTH);
                float alpha = 1f;
                ...
                if(colorWorld.A > 0)
                    MyTransparentGeometry.AddPointBillboard(leakInfo.MATERIAL_PARTICLE, (colorWorld * alpha).ToVector4() * 2, posWorld, particleData.Size, 0);

                if(colorOverlay.A > 0)
                    MyTransparentGeometry.AddPointBillboard(leakInfo.MATERIAL_PARTICLE, (colorOverlay * alpha), posOverlay, particleData.Size * LeakInfo.DRAW_DEPTH_F, 0);
            }
            ...
            for(int t = trails.Count - 1; t >= 0; --t)
            {
                ...
                if(pointVisible && trail.Life > 0)
                {
                    ...
                    var trailPosOverlay = camPos + ((trailPosWorld - camPos) * LeakInfo.DRAW_DEPTH);

                    var alpha = lifeRatio * 0.75f;
                    var size = 0.8f + (lifeRatio * 0.2f);

                    if(colorWorld.A > 0)
                        MyTransparentGeometry.AddPointBillboard(leakInfo.MATERIAL_PARTICLE, (colorWorld * alpha).ToVector4() * 2, trailPosWorld, particleData.Size * size, 0);

                    if(colorOverlay.A > 0)
                        MyTransparentGeometry.AddPointBillboard(leakInfo.MATERIAL_PARTICLE, (colorOverlay * alpha), trailPosOverlay, particleData.Size * size * LeakInfo.DRAW_DEPTH_F, 0);
                }
            }
