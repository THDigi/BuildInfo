...
namespace CoreSystems.Api
{
    ...
    public partial class CoreSystemsAPI
    {
        ...
        Action<ICollection<MyDefinitionId>> _getCoreWeapons;
        Action<ICollection<MyDefinitionId>> _getNpcSafeWeapons;
        Action<IDictionary<MyDefinitionId, List<MyTuple<int, MyTuple<MyDefinitionId, string, string, bool>>>>> _getAllWeaponMagazines;
        Action<IDictionary<MyDefinitionId, List<MyTuple<int, MyTuple<MyDefinitionId, string, string, bool>>>>> _getAllNpcSafeWeaponMagazines;

        Action<ICollection<MyDefinitionId>> _getCoreStaticLaunchers;
        Action<ICollection<MyDefinitionId>> _getCoreTurrets;
        Action<ICollection<MyDefinitionId>> _getCorePhantoms;
        Action<ICollection<MyDefinitionId>> _getCoreRifles;
        Action<IList<byte[]>> _getCoreArmors;

        Action<long, int, Action<ListReader<MyTuple<ulong, long, int, MyEntity, MyEntity, ListReader<MyTuple<Vector3D, object, float>>>>>> _registerDamageEvent;
        Func<long, bool, Func<MyEntity, IMyCharacter, long, int, bool>, bool> _targetFocusHandler;
        Func<long, bool, Func<IMyCharacter, long, int, bool>, bool> _hudHandler;
        Func<long, bool, Func<Vector3D, Vector3D, int, bool, object, int, int, int, bool>, bool> _shootHandler;
        Action<MyEntity, int, Action<int, bool>> _monitorEvents;
        Action<MyEntity, int, Action<int, bool>> _unmonitorEvents;
        Action<MyEntity, int, Action<long, int, ulong, long, Vector3D, bool>> _addProjectileMonitor;
        Action<MyEntity, int, Action<long, int, ulong, long, Vector3D, bool>> _removeProjectileMonitor;

        Func<MyEntity, object, int, double, bool> _shootRequest;

        Func<ulong, MyTuple<Vector3D, Vector3D, float, float, long, string>> _getProjectileState;
        Action<ulong, MyTuple<bool, Vector3D, Vector3D, float>> _setProjectileState;

        Action<MyEntity, ICollection<MyTuple<MyEntity, float>>> _getSortedThreats;
        Action<MyEntity, ICollection<MyEntity>> _getObstructions;
        Func<MyEntity, int, MyEntity> _getAiFocus;
        Func<MyEntity, MyEntity, int, bool> _setAiFocus;
        Func<MyEntity, long, bool> _releaseAiFocus;
        Func<MyEntity, bool> _hasAi;
        Func<MyEntity, bool> _hasCoreWeapon;

        Func<MyEntity, bool> _toggoleInfiniteResources;
        Action<MyEntity> _disableRequiredPower;

        Func<MyEntity, long> _getPlayerController;
        Func<MyEntity, MyTuple<bool, int, int>> _getProjectilesLockedOn;
        Action<MyEntity, ICollection<Vector3D>> _getProjectilesLockedOnPos;
        Func<MyDefinitionId, float> _getMaxPower;

        Func<MyEntity, float> _getOptimalDps;
        Func<MyEntity, float> _getConstructEffectiveDps;
        Func<MyEntity, MyTuple<bool, bool>> _isInRange;

        Func<MyEntity, int, MyTuple<bool, bool, bool, MyEntity>> _getWeaponTarget;
        Action<MyEntity, MyEntity, int> _setWeaponTarget;
        Action<MyEntity, bool, int> _fireWeaponOnce;
        Action<MyEntity, bool, bool, int> _toggleWeaponFire;
        Func<MyEntity, int, bool, bool, bool> _isWeaponReadyToFire;
        Func<MyEntity, int, float> _getMaxWeaponRange;
        Func<MyEntity, ICollection<string>, int, bool> _getTurretTargetTypes;
        Action<MyEntity, ICollection<string>, int> _setTurretTargetTypes;
        Action<MyEntity, float> _setBlockTrackingRange;
        Func<MyEntity, MyEntity, int, bool> _isTargetAligned;
        Func<MyEntity, MyEntity, int, MyTuple<bool, Vector3D?>> _isTargetAlignedExtended;
        Func<MyEntity, MyEntity, int, bool> _canShootTarget;
        Func<MyEntity, MyEntity, int, Vector3D?> _getPredictedTargetPos;
        Func<MyEntity, float> _getHeatLevel;
        Func<MyEntity, float> _currentPowerConsumption;
        Func<MyEntity, int, string> _getActiveAmmo;
        Action<MyEntity, int, string> _setActiveAmmo;

        Func<MyEntity, int, Matrix> _getWeaponAzimuthMatrix;
        Func<MyEntity, int, Matrix> _getWeaponElevationMatrix;
        Func<MyEntity, MyEntity, bool, bool, bool> _isTargetValid;
        Func<MyEntity, int, bool> _isWeaponShooting;
        Func<MyEntity, int, int> _getShotsFired;
        Action<MyEntity, int, List<MyTuple<Vector3D, Vector3D, Vector3D, Vector3D, MatrixD, MatrixD>>> _getMuzzleInfo;
        Func<MyEntity, int, MyTuple<Vector3D, Vector3D>> _getWeaponScope;
        Func<MyEntity, int, MyTuple<MyDefinitionId, string, string, bool>> _getMagazineMap;
        Func<MyEntity, int, MyDefinitionId, bool, bool> _setMagazine;
        Func<MyEntity, int, bool> _forceReload;
        ...
        /// <returns></returns>
        bool TargetFocusCallback(MyEntity target, IMyCharacter requestingCharacter, long handledEntityId, int modeCode)
        {
        ...
        /// <param name="modeCode"></param>
        /// <returns></returns>
        bool HudCallback(IMyCharacter requestingCharacter, long handledEntityId, int modeCode)
        {
        ...
        /// <returns></returns>
        bool ShootCallBack(Vector3D scopePos, Vector3D scopeDirection, int requestState, bool hasLos, object target, int currentAmmo, int remainingMags, int requestStage)
        {
        ...
        void ApiAssign(IReadOnlyDictionary<string, Delegate> delegates, bool getWeaponDefinitions = false)
        {
        ...
        void AssignMethod<T>(IReadOnlyDictionary<string, Delegate> delegates, string name, ref T field)
            where T : class
        {
            if(delegates == null)
            {
        ...
        public class DamageHandlerHelper
        {
            ...
            void DefaultCallBack(ListReader<MyTuple<ulong, long, int, MyEntity, MyEntity, ListReader<MyTuple<Vector3D, object, float>>>> listReader)
            {
                YourCallBackFunction(ProcessEvents(listReader));
                CleanUpEvents();
            }

            readonly List<ProjectileDamageEvent> _convertedObjects = new List<ProjectileDamageEvent>();
            readonly Stack<List<ProjectileDamageEvent.ProHit>> _hitPool = new Stack<List<ProjectileDamageEvent.ProHit>>(256);

            List<ProjectileDamageEvent> ProcessEvents(ListReader<MyTuple<ulong, long, int, MyEntity, MyEntity, ListReader<MyTuple<Vector3D, object, float>>>> projectiles)
            {
            ...
            readonly CoreSystemsAPI _wcApi;
            public DamageHandlerHelper(CoreSystemsAPI wcApi)
            {
                _wcApi = wcApi;
            }
